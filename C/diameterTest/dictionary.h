...
DiameterHeader diameterHeaderStruct;
int diameterHeaderStructOK=-1;
...
{
  int i=0;
  int p=DiameterHeaderLenMax;
  int len=diameterHeaderStruct.len;
  while(p<len){
...
void initDiameterHeader(char *packet)
{
	/*
	int version;
	int len;
	...
	*/
	...
	diameterHeaderStruct.version=getDiameterHeaderNum(DiameterHeaderVersion,packet);
	diameterHeaderStruct.len=getDiameterHeaderNum(DiameterHeaderMessageLenth,packet);
	diameterHeaderStruct.flags=getDiameterHeaderNum(DiameterHeaderFlags,packet)&0xff;
	diameterHeaderStruct.commandCode=getDiameterHeaderNum(DiameterHeaderCommandCode,packet);
	diameterHeaderStruct.applicationID=getDiameterHeaderNum(DiameterHeaderApplicationID,packet);
	diameterHeaderStruct.hopByHopID=getDiameterHeaderNum(DiameterHeaderHopByHop,packet);
	diameterHeaderStruct.endToEndID=getDiameterHeaderNum(DiameterHeaderEndToEnd,packet);
	diameterHeaderStructOK=1;
}
void initAvpIndex(char *packet)
{
	if(diameterHeaderStructOK!=1){
		printf("init Diameter Header not yet\n");
		return;
	}
	int pos=DiameterHeaderLenMax;
	int len=diameterHeaderStruct.len;
	//printf("1 len=%d\n",len);
...
void initDiameter(char *packet){
	initDiameterHeader(packet);
	initAvpIndex(packet);
}
